...
class FirebaseNotificationService extends LaravelServiceClass
{
     ...
     * @throws Exception
     */
    public function notifyUser($user_id, $title, $notification_body)
    {
        try {
            $message = CloudMessage::new();

            $tokens = $this->device_token_repository->whereIn('user_id', [$user_id]);
            ...
            if (count($tokens)){
                ...
                // we can only notify 500 at a time
                foreach ($device_tokens as $device_token) {
                    $this->notify($message, $device_token, $title, $notification_body);
                }
            }
